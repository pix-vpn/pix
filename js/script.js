...
;!function(j) {
    j(window).on("load", function() {
        j("#status").fadeOut(),
        j("#preloader").delay(350).fadeOut("slow"),
    ...
    (new WOW).init(),
    j(".mailchimp").length > 0 && j(".mailchimp").ajaxChimp({
        language: "es",
        callback: function(a) {
            "success" === a.result ? (j(".subscription-success").html(a.msg).fadeIn(1000),
            j(".subscription-error").fadeOut(500)) : "error" === a.result && j(".subscription-error").html(a.msg).fadeIn(1000)
        },
    ...
    j(".panel-group").on("hidden.bs.collapse", v),
    j(".panel-group").on("shown.bs.collapse", v);
